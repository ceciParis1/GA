...
# Fonction pour télécharger et vérifier si un fichier est bien un ZIP
def download_and_extract_shapefile(url, extract_to="shapefiles"):
    response = requests.get(url)
    
    # Vérifier si le téléchargement a réussi
    if response.status_code != 200:
        st.error(f"Erreur de téléchargement du fichier depuis {url}")
        return None
    ...
    try:
        ...
        st.error("Le fichier téléchargé n'est pas un ZIP valide.")
        return None
...
# Vérifier que les fichiers ont bien été téléchargés et extraits
if not commune_dir or not departement_dir:
    st.error("Le téléchargement ou la décompression des fichiers a échoué.")
else:
